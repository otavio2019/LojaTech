...
export function SaleForm({ customers, products }: { customers: Cliente[]; products: Produto[] }) {
  ...
  async function onSubmit(data: SaleFormData) {
    ...
    const response = await fetch("/api/vendas", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) });
    if (!response.ok) {
      const result = await response.json().catch(() => ({ error: "Erro ao registrar venda." }));
      setError("root", { message: result.error ?? "Erro ao registrar venda." });
      return;
    }
    setSuccess(true);
