...
export function DashboardView({ data }: { data: DashboardData }) {
  ...
  return <div className="space-y-8">
    ...
    <Card><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><h2 className="text-lg font-semibold">Exportar para o Power BI</h2><p className="mt-1 text-sm text-slate-400">Baixe os dados atuais em CSV e importe-os no Power BI Desktop.</p></div><div className="flex flex-wrap gap-2">{[["clientes", "Clientes"], ["produtos", "Produtos"], ["vendas", "Vendas"], ["calendario", "Calendário"]].map(([type, label]) => <a key={type} href={`/api/export?tipo=${type}`} className="rounded-xl border border-cyan-400/30 px-3 py-2 text-sm font-medium text-cyan-300 transition hover:border-cyan-300 hover:bg-cyan-400/10">Baixar {label}</a>)}</div></div></Card>
